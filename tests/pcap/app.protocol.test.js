...
assert.ok(page.includes('v0.4.2'), 'the stable release version is shown');
...
assert.ok(page.includes('/releases/download/v0.4.2/pcap-agent-windows.zip'), 'Windows download targets the packaged stable asset');
assert.ok(page.includes('/releases/download/v0.4.2/pcap-agent-linux.tar.gz'), 'Linux download targets the packaged stable asset');
assert.ok(page.includes('/releases/download/v0.4.2/pcap-agent-macos-intel.tar.gz'), 'macOS Intel download targets the packaged stable asset');
assert.ok(page.includes('/releases/download/v0.4.2/pcap-agent-macos-apple-silicon.tar.gz'), 'macOS Apple Silicon download targets the packaged stable asset');
const bootstrapIndex = page.indexOf('window.__AHLYX_RELAY_BOOTSTRAP');
const vercelAnalyticsIndex = page.indexOf('/_vercel/insights/script.js');
const googleAnalyticsIndex = page.indexOf('googletagmanager.com/gtag/js');
assert.ok(bootstrapIndex >= 0 && bootstrapIndex < vercelAnalyticsIndex && bootstrapIndex < googleAnalyticsIndex,
    'relay fragment cleanup bootstrap appears before every analytics script');
